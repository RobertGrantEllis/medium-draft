...
/*
Adds a new block at the current cursor position with the designated `newType` and `initialData`.
If the current block is empty, replaces the current empty block instead.
*/
export const addNewBlock = (editorState, newType = Block.UNSTYLED, initialData = {}) => {
  ...
  if (currentBlock.getLength() > 0) {
    ...
    return addNewBlockAt(editorState, key, newType, initialData);
  }

  // leverage existing empty block
